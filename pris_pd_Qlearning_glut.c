...
void save_snapshots(int step, int *s){
	...
	sprintf(output_snaps_freq, "data/two-actions-sctb-c-or-d/snapshots/SnapshotStep%d_T%.2f_S_%.2f_LSIZE%d_rho%.5f_P_DIFFUSION%.2f_CONF_%d_%ld_prof.dat",
...
void file_initialization(void)
{
	...
	sprintf(output_file_freq,"data/two-actions-sctb-c-or-d/T%.2f_S_%.2f_LSIZE%d_rho%.5f_P_DIFFUSION%.2f_CONF_%d_%ld_prof.dat",
